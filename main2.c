...
static inline uint16_t sprd_get_frame_type(sprd_context_t *sprd_context) {
    return (sprd_context->buffer[0] << 8) | sprd_context->buffer[1];
}

static inline uint16_t sprd_get_frame_data_size(sprd_context_t *sprd_context) {
    return (sprd_context->buffer[2] << 8) | sprd_context->buffer[3];
}

static inline uint8_t *sprd_get_frame_data(sprd_context_t *sprd_context) {
...
static int sprd_send(sprd_context_t *sprd_context) {
    uint8_t buffer[FRAME_MAX_SIZE];
    int buffer_length = 0;
...
static int sprd_receive(sprd_context_t *sprd_context) {
    uint8_t buffer[FRAME_MAX_SIZE];
...
static int sprd_send_frame(sprd_context_t *sprd_context, uint16_t type, uint16_t data_size, uint8_t *data) {
...
static int sprd_send_and_check_frame(sprd_context_t *sprd_context, uint16_t type, uint16_t data_size, uint8_t *data) {
    int ret;
...
static int sprd_send_usb_hello(sprd_context_t *sprd_context) {
    int ret;
    ...
    // type: CALIBERATE_COMMOND_T
    uint8_t data[10] = {HDLC_FLAG, 0, 0, 0, 0, 8, 0, 0xfe, 0, HDLC_FLAG};
    data[8] = 0x02; // avoid: 0x17, 0x15, calibration=2,0,146
    ...
    printf("Sent HDLC_FLAG, waiting for response...\n");
    ...
    printf("Sending second part of USB hello...\n");
    uint8_t data2[] = { 0x7e,0,0,0,0,0x20,0,0x68,0,0x41,0x54,0x2b,0x53,0x50,0x52,0x45,0x46,0x3d,0x22,0x41,0x55,0x54,0x4f,0x44,0x4c,0x4f,0x41,0x44,0x45,0x52,0x22,0xd,0xa,0x7e };
    ret = libusb_bulk_transfer(
            sprd_context->handle,
            sprd_context->ep_out,
            data2,
...
static int sprd_check_connection(sprd_context_t *sprd_context) {
 ...
 */
static int sprd_execute_payload(sprd_context_t *sprd_context, uint32_t load_address, uint8_t *payload, uint32_t payload_size) {
    int ret;
    uint8_t buffer[8];
...
static int sprd_do_work(sprd_context_t *context) {
    int ret;

    context->crc = sprd_fdl_crc;
    ...
    ret = sprd_send_usb_hello(context);
    if (ret) {
        printf("sprd_send_usb_init failed: %s\n", libusb_error_name(ret));
        return ret;
    }

    printf("Received BSL_REP_VER: %.*s\n", sprd_get_frame_data_size(context), sprd_get_frame_data(context));

    ret = sprd_check_connection(context);
    if (ret) {
    ...
    ret = sprd_execute_payload(context, 0x00005500, payload, payload_size);
    if (ret) {
    ...
    void *data;
    if (da_receive(context->handle, &cmd, &data_length, &data)) {
    ...
    ret = da_emmc_init(context->handle);
    if (ret) {
    ...
    uint32_t sec_count;
    ret = da_get_sec_count(context->handle, &sec_count);
    if (ret) {
    ...
    ret = da_read_single_block(context->handle, 1, blk_buf);
    if (ret) {
        printf("da_read_single_block failed: %d\n", ret);
        return -1;
    }
    ...
    for (uint32_t i = 0; i < partition_entries_blocks; i++) {
        ...
        ret = da_read_single_block(context->handle, lba, blk_buf);
        if (ret) {
            printf("da_read_single_block failed: %d\n", ret);
            return -1;
        }
        ...
        for (uint32_t j = 0; j < partition_entries_per_block; j++) {
            ...
            read_partition_to_file(context->handle, &gpt_entry);
        }
    }
